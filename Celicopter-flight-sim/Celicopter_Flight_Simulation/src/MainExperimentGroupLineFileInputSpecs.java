...
public class MainExperimentGroupLineFileInputSpecs extends JPanel implements Runnable{

	/**Special thingy*/
	private static final long serialVersionUID = -6394357674966065801L;
...
//				fileReader.nextLine();
	...
	public void moveObjects(){
		...
		if(delta2>=ITERATIONS_DELAY){
			...
			return;
				 ...
				 target.move((int) Math.round(targetPositions[targetPositionIndex]+screenDimentions.getWidth()/2), (int) Math.round(screenDimentions.getHeight()/2), screenDimentions.width, screenDimentions.height);
				 targetPositionIndex++;
			}
	...
	public void drawObjectsOnScreen(Graphics2D g){
		...
		if(!isCalibrated){
			...
			//g.setColor(Color.black);
			target.draw(g,screenDimentions.width,screenDimentions.height);
			g.setColor(Color.red);
			warfighter.draw(g);
		}
	}
	...
	public static void main(String[] args){
		...
		MainExperimentGroupLineFileInputSpecs m=new MainExperimentGroupLineFileInputSpecs();
		
		//While the program is still running, wait to close the file writer
		while(m.isRunning){
			try{Thread.sleep(MainExperimentGroupLineFileInputSpecs.DELAY_TIME);} catch(InterruptedException e) {}
		}
