...
public class MainExperimentGroup extends JPanel implements Runnable{
	...
	protected static final double[] SPATIAL_FREQUENCIES={16,1000};
	...
	protected static final double[] MODULATIONS={0.1,1.0};
...
//				str.
	...
	public void moveObjects(){
		...
		if(delta2>=ITERATIONS_DELAY){
			...
			return;
				...
				target.move((int) Math.round(targetPositions[targetPositionIndex]+3*screenDimentions.getWidth()/4), (int) Math.round(screenDimentions.getHeight()/2), screenDimentions.width, screenDimentions.height);
				targetPositionIndex++;
			 }
			 else {
	...
	public void initObjects(){
		...
		Target t1=new Target(1,1,-0.3,0.2,3,1,1);
		Target t2=new Target(2,5,0,0,98,1,2);
		Target t3=new Target(5,325,0,0,5,1);
		double[] radii={300,300,300,300};
		double[] thetas={0,90,180,270};
		Target[] scs={t1,t2,t3,new Target()};
		target=new TargetGroup(300,400,0.2,0.4,radii,thetas,scs);
	...
	public static void main(String[] args){
		...
		MainExperimentGroup m=new MainExperimentGroup();
		while(m.isRunning){
			try{Thread.sleep(MainExperimentGroup.DELAY_TIME);} catch(InterruptedException e) {}
		}
